    ...
    for (const favorite of newFavorites) {
      ...
      try {
        ...
        if (response.status === 201) {
          console.log(`✅ Favorite item added to the database:`, response.data);
        } else {
          console.log(`❌ API responded but item was not added successfully.`);
        }
      } catch (error) {
...
const getFavoritesForStore = async (
  ...
  try {
    ...
    if (!favorites) {
      console.log(`Cache is empty for store ${store_id}, fetching from API...`);
